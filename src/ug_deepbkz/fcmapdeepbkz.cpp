...
/**@file    fcmapdeepbkz.cpp
 * @brief   FiberCMapDeepBKZ MAIN.
      ...
      )
{
   ...
   std::cout << "syntax: " << argv[0]
             << " fcmapdeepbkz_param_file problem_file_name "
      ...
      )
{
   ...
   CMapDeepBkzParaSolver paraSolver(argc, argv, comm.get(), localComm[ solverThreadData->rank - 1 ], paraParams.get(), paraInstance, detTimer.get(), paraTimer->getElapsedTime());
...
// #endif
   ...
   paraLc.reset(new CMapDeepBkzParaLoadCoordinator(comm.get(), paraParams.get(), &paraInitiator, &racingSolversExist, paraTimer.get(), detTimer.get(), nThreadsPerRank));
