...
template <typename T> struct GSapprox {
    ...
    void gslentofile(std::string filename,std::ios::openmode option=std::ios::trunc) {
        std::ofstream of;
        of.open(filename.c_str(),option);
        for (int i=1;i<std::max(c.size(),cd.size());i++) {
            of << i << "\t";
            if (i<c.size()) of << c[i];
            of << "\t";
...
template <typename GFLOAT,typename MAT,typename VEC> struct LatticeBasisBase {
    ...
    VEC& operator [] (int i) {
        if ((0<=i) && (i<L.NumRows())) {
            ...
            std::cout << "range_error: " << i << " size=" << dim << std::endl;
            exit(0);
        }
    }
    ...
    void updateGSBasis(int istart,int iend) {
        dim = L.NumRows();
        memoryalloc();
        istart = std::max(istart,GScomputed+1);
...
template <typename T> void display_vector(std::vector<T>& a,int istart=0,int iend=0) {
    ...
    std::cout << "[";
    for (int i=istart;i<=iend;i++) std::cout << a[i] << " ";
    std::cout << "]" << std::endl;
}
