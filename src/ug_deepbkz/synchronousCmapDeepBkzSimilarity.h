...
/**@file    synchronousCmapDeepBkzSimilarity.h
 * @brief   Calculation of similarity of bases for synchronousCmapdeepbkz.
...
#ifndef __SYNCHRONOUS_CMAP_DEEPBKZ_SIMILARITY_H__
#define __SYNCHRONOUS_CMAP_DEEPBKZ_SIMILARITY_H__
...
{
   GSO = MatrixXi(basis).cast<double>()
      .transpose()
      .householderQr()
      .householderQ()
      .transpose();
...
///
/// @brief Grassmann Chordal Metric = projection frobenius metric
/// @param[in] cc canonicalCorrelations
/// @details 2^(-1/2) frobenius_norm(GSOA*GSOA^T - GSOB*GSOB^T)
///          = sqrt(sum(sin^2(theta_i) for 0 <= i < m))
///          = sqrt(sum(1 - cos^2(theta_i) for 0 <= i < m))
...
///
double
grassmann_chordal_2norm_metric(
      Eigen::VectorXd &cc
      )
{
   int m = cc.size();
...
///
double
grassmann_chordal_fnorm_metric(
      Eigen::VectorXd &cc
      )
{
   int m = cc.size();
...
///
double
grassmann_projection_2norm_metric(
      Eigen::VectorXd &cc
      )
{
   int m = cc.size();
      ...
      )
{
   ...
   int barWidth = 50;
   auto setProgressorCallback = [=](int _k){
      double progress = static_cast<double>(_k+1) / nSamples;
      ...
      std::cout << "] " << static_cast<int>(progress * 100.0) << " % (" << _k+1 << " / " << nSamples << ") \r";
      std::cout.flush();
   };
   ...
   for( k = 0; k < nSamples; ++k )
   {
      ...
      for( int l = grassmannIndexes.size()-1; l > -1; --l )
      {
         ...
         if( d >= n / 2.0 )
         {
            ...
            JacobiSVD<Eigen::MatrixXd> SVD{subGSOA * subGSOB.transpose()};
...
{
   std::deque<MatrixXi*> _basisDeque;
   for( int i = basisDeque.size()-1; i >= 0; --i )
   {
      _basisDeque.push_back(&(basisDeque.at(i)));
   }
   return outputSimilarityOfBasis(_basisDeque, time, n, numThreads, verbose);
}
