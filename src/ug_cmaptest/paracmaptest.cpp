...
/**@file    paracmaptest.cpp
 * @brief   CMAP-TEST MAIN.
...
{
   std::cout << std::endl;
   std::cout << "syntax: " << argv[0] << " #MPI_processes(#solvers + 1) paracmaptest_param_file problem_file_name "
      ...
      )
{
   ...
   CMapTestParaSolver paraSolver(argc, argv, comm, localComm.get(), paraParams.get(), paraInstance, detTimer.get(), paraTimer->getElapsedTime());
...
// #endif
   ...
   if( comm->getRank() == 0 )
   {
      ...
      paraLc.reset(new CMapLapParaLoadCoordinator(comm, paraParams.get(), &paraInitiator, &racingSolversExist, paraTimer.get(), detTimer.get(), nThreadsPerRank));
