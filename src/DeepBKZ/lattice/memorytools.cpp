...
namespace shared_memory {
    ...
    template <typename T> void* allocate1(int id,int size,int vl=0) {
        ...
        if (vl!=0) {
            std::cout << "---------------------------" << std::endl;
            std::cout << "memoryalloc: type=" << typeid(T).name() << " id=" << id << std::endl;
            std::cout << "thash=" << ti.thash << std::endl;
        }        
        darray dd;
        if (memory.find(ti) == memory.end()) {
            if (vl!=0) std::cout << "not found" << std::endl;
            //not exist
        } else {
            dd = memory.at(ti);
        }
        if (vl!=0) std::cout << "allocated size=" << dd.size1 << std::endl;
        ...
        if (dd.size1 < size) {
            ...
            dd.size1 = size;
            if (vl!=0) std::cout << "allocate new memory of type=" << typeid(T).name() << " size=" << size << std::endl;
        }
        memory[ti] = dd;
        return (void*)dd.d;
    }
    ...
    template <typename T> void* allocate2(int id,int s1,int s2,int vl=0) {
        ...
        if ((dd.size1 < s1) || (dd.size2 < s2)){
            ...
            dd.size1 = s1;
            dd.size2 = s2;
            if (vl!=0) std::cout << "allocate new memory of type=" << typeid(T).name() << " size=" << s1 << ","  << s2<< std::endl;
        }
        memory[ti] = dd;
        return (void*)dd.d;
    }
    ...
    template <typename T> void* allocate3(int id,int s1,int s2,int s3,int vl=0) {
        ...
        if ((dd.size1 < s1) || (dd.size2 < s2) || (dd.size3 < s3)) {
            ...
            dd.size2 = s2;
            dd.size3 = s3;
            if (vl!=0) std::cout << "allocate new memory of type=" << typeid(T).name() << " size=" << s1 << "," << s2 << "," << s3 << std::endl;
        }
        memory[ti] = dd;
        return (void*)dd.d;
    }
